...
dish_size=1.0 # in meters
Trx='1e3' # receiver temp in mK
hrsperday=24.0
numdays=1825.0
totalNumHrs=`echo "scale=5; $hrsperday * $numdays" | bc -l`
sqSidelength=1280 # number of dishes on one side of the square
sqSpacing=1.0 # distance between dish centers in meters
...
#   echo $currentZ
    ...
    python calc_sense_z_cylind.py -m $fgOption -T $currentTsky -R $restFreq -f $currentFreq --bwidth=$currentBandwidth $current_array_fname --n_per_day=$hrsperday --ndays=$numdays --eor="temp_Pk.npz"
    rm temp_Pk.npz
